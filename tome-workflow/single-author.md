...
4. For a phase, run `python3 tools/report_tome_progress.py BUILD_ID N validating`. For a
   Phase-3 section, run `python3 tools/report_section_progress.py BUILD_ID sNN INDEX TOTAL validating`.
   Then **stop your turn**.
5. The harness runs the exact gate below. It marks a clean unit complete and resumes this same
   warm session with the next unit. If the gate fails, it resumes this session with the complete
   report; repair only the assigned unit, mark it validating again, and stop again.

Never run a validator or a deterministic phase-transition command yourself in single-author
mode. Never mark a unit complete yourself. Those actions belong to the harness, which prevents
you from starting later work before the current mechanical gate is clean.
...
- Phase 1: the harness runs `python3 tools/validate_tome.py tomes/BUILD_ID --phase-1-plan .tome-build/BUILD_ID.plan.md`,
...
- Phase 3: author one whole section at a time in Arc order. The harness sets its `authoring`
  marker before resuming you. After authoring, report `validating` and stop. The harness runs
...
- Phase 7: the harness runs
  `python3 tools/validate_phase3.py tomes/CURRENT_TOME --plan .tome-build/BUILD_ID.plan.md --tooling TOOLING --strict`,
  then `python3 tools/smoke_tome.py CURRENT_TOME`. Both must exit 0.
- Phase 8: do the semantic first-time-student review yourself and mark the phase validating.
  The harness repeats the Phase-7 strict and smoke commands. A clean validator is necessary but
  does not replace the complete semantic review.
