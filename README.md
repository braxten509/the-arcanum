...
- `global-configs/` — cross-tome, platform-level tuning TOMLs: `sigil.toml` (the drawn spell sigil + its sound), `particles.toml` (cast bursts), `audio.toml` (every GhostAudio sound knob), `settings.toml` (reader-wide settings — audio, pen, AI models/keys, Pushover creds; the settings panel rewrites it on save), and `runtimes/<name>.toml` (one per language — add a TOML to add a language, no Python ever); edit and refresh, no rebuild
