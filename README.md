...
- `tools/` — author tooling: `build_tome.py` (one persistent, pauseable AI session for Phases 1–8), `validate_tome.py <tome>` (machine-checks a tome against `tome-authoring/`; exits 1 on any error, CI-friendly), `new_tome.py <id>` (scaffolds a new split-layout tome), `scaffold_tome_sections.py <tome> --plan <plan>` (turns an approved Arc into Phase 2's placeholder tree), `split_tome.py <tome>` (converts a flat tome to split layout), and `gen_attacks.py <tome>` (regenerates attacks through the live server). The author stays in one resumable CLI session and validates after every phase and every Phase-3 section.
