"""Resolve checked-in permission profiles for runtime and authoring roles."""
...
def profile_paths(name, *, build_id, tome_id, phase, section_id="",
                  previous_section_id="", section_index="", section_count="",
                  tooling="", runtime_id=""):
    profile = os.path.join(
        ROOT, "global-configs", "permissions", f"{name}.toml")
    try:
        with open(profile, "rb") as handle:
            value = tomllib.load(handle)
    except (OSError, tomllib.TOMLDecodeError) as exc:
        raise RuntimeError(
            f"invalid permission profile {name!r}: {exc}") from exc
    values = {
    ...
    rows.extend(
        row for row in value.get("phase") or []
    ...
    for row in rows:
        for access in ("read", "write", "both", "execute"):
            for item in row.get(access) or []:
                command = (
                    item if access == "execute" and isinstance(item, list)
                    else [item])
                if not command or any(
                        not isinstance(part, str) or not part
                        for part in command):
                    raise RuntimeError(
                        f"permission profile {name!r} has an invalid "
                        f"{access} entry")
                try:
                    command = [part.format_map(values) for part in command]
                except KeyError as exc:
                    raise RuntimeError(
                        f"permission profile {name!r} has unknown "
                ...
                for relative in command:
                    if os.path.isabs(relative) or ".." in relative.split("/"):
                        continue
                    for candidate in glob.glob(os.path.join(ROOT, relative)):
                        path = os.path.realpath(candidate)
                        if os.path.commonpath((path, ROOT)) != ROOT:
                            raise RuntimeError(
                                f"permission profile {name!r} escapes "
                                "the repository")
    ...
    for access in ("read", "write", "both", "execute"):
        ...
        for item in (value.get("system") or {}).get(access) or []:
            if not isinstance(item, str):
                raise RuntimeError(
                    f"permission profile {name!r} has a non-text system path")
            path = item.format_map(values)
            if not os.path.isabs(path) or path in ("/", "/home"):
                raise RuntimeError(
                    f"permission profile {name!r} has unsafe system path "
                    f"{path!r}")
            if path.startswith("/tmp/arcanum/") and access in ("write", "both"):
                from .agent_scratch import prepare
                prepare(build_id)
