...
ARC_CONTRACT = (
    ...
    "Language practice allocation (one physical semicolon-separated line with every\n"
    "section exactly once as `s01 = language-*[, language-*]`; every section receives\n"
    "at least one honest capability from the declared spine that its Working can materially\n"
    "exercise while advancing its sealed milestone; tooling, framework, build, packaging,\n"
    "or story activity alone is never language practice);\n"
    ...
    "requirements = path|none`; runtime names the source entrypoint and always uses\n"
    "requirements = none—the requirements slot is not a generic source inventory or build\n"
    "entrypoint, so other clean-rebuild files stay in Artifact ownership/lifecycle and Acceptance\n"
    "proof; select package whenever the Arc promises a packaged,\n"
