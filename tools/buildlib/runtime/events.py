"""Shared structured-event parsing for provider CLI invocations."""
import json
...
def usage_from_line(line):
    ...
    # Anthropic reports fresh, cache-read, and cache-created inputs as disjoint
    # counters. OpenAI reports cached input as a subset of input_tokens.
...
def session_id_from_line(line):
    """Read a provider session/thread id from a structured event."""
    try:
        row = json.loads(line)
    except ValueError:
        return ""
    ...
    part = row.get("part")
    return str(
        row.get("thread_id") or row.get("session_id") or row.get("sessionID")
