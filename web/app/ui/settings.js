...
export function showStudySettings() {
  ...
  const skins = tome().skins || [];
  const skinOpt = (s) => [s.id, (s.name || s.id).toUpperCase(), s.css ? "(skin)" : "(theme)"];
  const themeOpts = [
    ...skins.filter((s) => s.id === "vellum").map(skinOpt),
    ...(tome().themes || [])
    .filter((t) => getState().themes && getState().themes[t.id]).map((t) => [t.id, (t.name || t.id).toUpperCase(), ""]),
    ...skins.filter((s) => s.id !== "vellum").map(skinOpt)];
  modal(`<h2>TRIM THE WICK</h2>
    ...
    <div style="display:flex;align-items:center;gap:10px;margin-top:6px">
      ...
      <select id="as-theme" class="cfg-select" style="flex:1">${themeOpts.map(([id, name, suffix]) =>
        `<option value="${id}"${suffix ? ` data-suffix="${suffix}"` : ""}${getState().theme === id ? " selected" : ""}>${name}</option>`).join("")}</select>
    </div>
    ...
    <label class="choice" style="margin-top:6px"><input type="checkbox" id="as-pen-trials" ${getState().pen.trials !== false ? "checked" : ""}><span><b>TRIALS</b> <span class="dim">— the answers you write into a trial</span></span></label>
    <label class="choice" style="margin-top:6px"><input type="checkbox" id="as-pen-drill" ${getState().pen.drill !== false ? "checked" : ""}><span><b>COPYING DRILL</b> <span class="dim">— the box you copy incantations into</span></span></label>
    ...
    <div style="display:flex;align-items:center;gap:10px;margin-top:6px">
      ...
      <input type="range" id="as-mvol" class="vol-slider" min="0" max="100" value="${getState().audio.volume}" style="flex:1">
      <span class="num dim" id="as-mvol-n" style="width:44px;text-align:right">${getState().audio.volume}%</span>
    </div>
    <div style="display:flex;align-items:center;gap:10px;margin-top:6px">
      <span class="dim" style="font-size:12px;width:78px">WIND</span>
      <input type="range" id="as-wvol" class="vol-slider" min="0" max="100" value="${getState().audio.wind}" style="flex:1">
      <span class="num dim" id="as-wvol-n" style="width:44px;text-align:right">${getState().audio.wind}%</span>
    </div>
    ...
    <div style="display:flex;align-items:center;gap:10px;margin-top:6px">
      <span class="dim" style="font-size:12px;width:78px">VOLUME</span>
      <input type="range" id="as-uvol" class="vol-slider" min="0" max="200" value="${getState().audio.ui}" style="flex:1">
      <span class="num dim" id="as-uvol-n" style="width:44px;text-align:right">${getState().audio.ui}%</span>
    </div>
  ...
  mvol.oninput = () => {
    getState().audio.volume = +mvol.value;
    $("#as-mvol-n", root).textContent = getState().audio.volume + "%";
    GhostAudio.setVolume(getState().audio.volume);
    save();
  };
  const wvol = $("#as-wvol", root);
  wvol.oninput = () => {
    getState().audio.wind = +wvol.value;
    $("#as-wvol-n", root).textContent = getState().audio.wind + "%";
    GhostAudio.setWind(getState().audio.wind);
    save();
  };
  $("#as-pen-trials", root).onchange = (e) => { getState().pen.trials = e.target.checked; applyPen(); save(); };
  $("#as-pen-drill", root).onchange = (e) => { getState().pen.drill = e.target.checked; applyPen(); save(); };
  ...
  uvol.oninput = () => {
    getState().audio.ui = +uvol.value;
    $("#as-uvol-n", root).textContent = getState().audio.ui + "%";
    GhostAudio.setUiVol(getState().audio.ui);
    GhostAudio.sfx("click");
    save();
  };
  // main grader: provider select + model combo / key / custom command
  const GRADER_DEFAULTS = { "claude-cli": "claude-opus-4-8", "antigravity-cli": "Gemini 3.1 Pro (High)", "codex-cli": "", "opencode-cli": "opencode-go/deepseek-v4-flash", anthropic: "claude-opus-4-8", openai: "gpt-5.1", ollama: getState().ai.grader, other: "" };
  ...
  const TOME_AI = (tome().defaults && tome().defaults.ai) || {};
  let PROVIDERS = {};
  let ollamaModels = [];
  ...
  function fillModelOptions() {
    const kind = getState().ai.graderKind, saved = getState().ai.graderModel || "";
    const opts = [];
  ...
  const paintGrader = () => {
    const kind = getState().ai.graderKind;
    gkind.value = kind;
    ...
    if (needsKey) gkey.value = getState().ai.keys[kind] || "";
    if (isOther) { gcustomRow.style.display = "none"; gcmd.value = getState().ai.graderCommand || ""; }
    else fillModelOptions();
  };
  gkind.onchange = () => {
    getState().ai.graderKind = gkind.value;
    getState().ai.graderModel = GRADER_DEFAULTS[gkind.value] || "";
    paintGrader(); save();
  };
  gmodelSel.onchange = () => {
    if (gmodelSel.value === CUSTOM) { gcustomRow.style.display = "flex"; gcustom.focus(); getState().ai.graderModel = gcustom.value.trim(); }
    else { gcustomRow.style.display = "none"; getState().ai.graderModel = gmodelSel.value; }
    save();
  };
  gcustom.onchange = () => { getState().ai.graderModel = gcustom.value.trim(); save(); };
  gkey.onchange = () => { getState().ai.keys[getState().ai.graderKind] = gkey.value.trim(); save(); };
  gcmd.onchange = () => { getState().ai.graderCommand = gcmd.value.trim(); save(); };
  paintGrader();
  ...
  apiFetch("/api/models").then((r) => r.json()).then((d) => {
    ...
    if (getState().ai.graderKind !== "other") fillModelOptions();  // repaint with the live census
    ...
    const cur = `${getState().ai.oracleKind || "ollama"}:${getState().ai.oracle}`;
    ...
    if (!oopts.some(([v]) => v === cur)) oopts.unshift([cur, getState().ai.oracle, "(not installed)"]);
    ...
    osel.onchange = () => {
      ...
      getState().ai.oracleKind = v.slice(0, i);
      getState().ai.oracle = v.slice(i + 1);
      save();
    };
    // THE STAND-IN JUDGE — local ollama models only
    const gsel = $("#as-grader", root);
    const names = list.map((m) => m.name);
    if (!names.includes(getState().ai.grader)) names.unshift(getState().ai.grader); // keep saved value even if uninstalled
    gsel.innerHTML = names.map((n) => {
      const m = list.find((x) => x.name === n);
      return `<option value="${esc(n)}"${n === getState().ai.grader ? " selected" : ""}>${esc(n)}${m ? ` — ${m.gb} GB` : " (not installed)"}</option>`;
    }).join("");
    gsel.onchange = () => { getState().ai.grader = gsel.value; save(); };
